...
async def bot_status_msg(c: Client, m: Message):
    if not is_bot_online():
        ...
        await m.reply_text(
            f"<b>⚠ Bot em manutenção, tente novamente mais tarde. Para saber as novidades do bot, entre no grupo ou canal de noticias antes da manutenção.</b>"
        )
...
async def bot_status_cq(c: Client, m: CallbackQuery):
    if not is_bot_online():
        news_user = get_news_user()
        await m.answer(
            f"<b>⚠ Bot em manutenção, tente novamente mais tarde.</b> <b>Para saber as novidades do bot, entre no grupo: t.me/tropadolzz e aguarde novas transmissões<b>.",
            show_alert=True,
            cache_time=5,
        )
        await m.stop_propagation()
        ...
        results = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    f"<b>⚠ Bot em manutenção, tente novamente mais tarde.</b> <b>Para saber as novidades do bot, entre no grupo: t.me/cardingsdo7 e aguarde novas transmissões<b>."
                ),
            )
        ]
