...
async def verify_pay(objeto, c: Client, send, value: float):
    ...
    txt_info_user = f"""📛 Ops! o usuario: {mention}
...
async def auto_pay(c: Client, m: Union[CallbackQuery, Message]):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="❮ ❮", callback_data="add_balance")]
        ]
    )
    ...
    if not select_pay:
        if isinstance(m, CallbackQuery):
            return await m.answer(
                "⚠️ Pix automático desativado, use o pix manual.", show_alert=True
            )
        else:
            return m.reply_text(
                "⚠️ Pix automático desativado, use o pix manual.", reply_markup=kb
            )
    ...
    if isinstance(m, CallbackQuery):
        ...
        cob = await m.message.ask(
            f"<b>💠 Digite o valor para ser cobrado (mínimo {min_value}), ex.: 10 ou 12.00:</b>",
        ...
        if len(m.command) == 1:
            return await m.reply_text(
                "<b>💠 Digite com um valor. ex: /pix 5</b>", reply_markup=kb
            )
    ...
    if None in (dados[0], dados[2]) and select_pay == "mercado pago":
        cpf, full_name = "0", "0"
        await mm.reply_text(
            "⚠️ Você não possui dados cadastrados, por favor responda o formulario a seguir.\n"
        ...
        email = await mm.ask("<b>📧 E-mail: (pode ser dados aleatorio exemplo: soucorno@gmail.com)</b>", reply_markup=ForceReply(), timeout=120)
        ativ = 0
        if ativ:
            full_name = await mm.ask(
                "<b>👤 Nome completo do pagador: (pode ser dados aleatorio exemplo: wilma bernadete de almeida borba)</b>",
                reply_markup=ForceReply(),
                timeout=120,
            )
        ...
        for _ in range(3):
            cpf = await mm.ask(
                "<b>👤 CPF da lara irá pagar: (pode ser dados aleatorio exemplo: 30599385120)</b>",
                reply_markup=ForceReply(),
                timeout=120,
            )
    ...
    base = f"""✅ <b> Obaaa! O seu pagamento foi gerado com sucesso !</b>

<b>💵 Valor a ser pago: </b><i> R$ {float(value)}</i>

<b>📩 Clique no código Pix Cópia-Cola para realizar o pagamento pelo app do seu banco.</b>

<code>{qr}</code>

<b>⚠️ Se o pagamento não for pago, o seu acesso poderá ser bloqueado automáticamente por SPAM em nosso bot.</b>

"""

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🔔 Ja fiz o pagamento", callback_data="start")]
        ]
    )
