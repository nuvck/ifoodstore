...
from config import ADMIN_CHAT, LOG_CHAT
...
async def comprar_cc_list(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton("💳 FULL DADOS", callback_data="comprar_cc unit"),
    ...
    await m.edit_message_text(
        f"""<b>[ ](https://i.ibb.co/4VtX22P/credit-card.gif)💳 Comprar</b>
<i>- Escolha no menu abaixo o que você quer comprar.
     
Avisos
- Checker disponível</i>
...
async def search_cc(c: Client, m: InlineQuery):
    ...
    if typ not in ("bin", "banco", "bandeira"):
        return
    ...
    rt = cur.execute(
        f"SELECT number, month, year, {typ2} FROM cards WHERE {typ2} LIKE ? AND pending = 0 ORDER BY RANDOM() LIMIT 50",
        [qry.upper()],
    ).fetchall()
...
async def comprar_ccs(c: Client, m: CallbackQuery):
    ...
    await m.edit_message_text(
        f"""[ ](https://i.ibb.co/ZfGdx3v/Cards2-gif.gif)<b>💳 Comprar CC FULL DADOS</b>
