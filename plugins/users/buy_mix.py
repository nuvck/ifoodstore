...
async def buy_mixes(c: Client, m: CallbackQuery):
    ...
    if not levels_list:
        return await m.answer(
            "⚠️ Não há Mixes disponíveis no momento, tente novamente mais tarde.",
            show_alert=True,
        )
...
async def buy_mix(c: Client, m: CallbackQuery):
    ...
    if len(sold_list) < quantity:
        return await m.edit_message_text(
            "⚠️ Infelizmente não consegui completar sua requisição de mix.",
            reply_markup=kb,
        )
    ...
    await c.send_document(LOG_CHAT, file)
