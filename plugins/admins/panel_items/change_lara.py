...
async def change_lara_details(c: Client, m: CallbackQuery):
    ...
    name = await m.message.ask("<b>👤 Informe o nome da lara:</b>", reply_markup=ForceReply())
    pix_key = await m.message.ask(
        "<b>💠 Informe a chave Pix da lara:</b>", reply_markup=ForceReply()
    )
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton("<b>✅ Ok</b>" , callback_data="painel")],
        ]
    )

    await m.message.reply_text(
        "<b>✅ Dados da lara alterados com sucesso.</b>", reply_markup=kb
    )
