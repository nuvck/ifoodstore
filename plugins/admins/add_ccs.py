...
async def on_add_m(c: Client, m: Message):
    ...
    if cards:
        ...
        if not total:
            text = (
                "<b>❲❓❳ - ão encontrei CCs na sua mensagem. Envie elas como texto ou arquivo.</b>"
            )
        else:
    ...
    await m.reply_text(
        "💳 Modo de adição ativo. Envie as CCs como texto ou arquivo no formato CC|DATA|CVV|NOME|CPF e elas serão adicionadas.",
    ...
    while True:
        if not first:
            await m.reply_text(
                "↘️ Envie mais CCs ou digite /done para cancelar o modo de adição.",
                reply_markup=ForceReply(),
            )
        ...
        try:
            ...
            await m.reply_text(
                "<b>❲❓❳ - Não recebi uma resposta para o comando anterior e ele foi automaticamente cancelado.</b>",
                reply_markup=kb,
            )
            return
        ...
        if not msg.text and (
            ...
            await msg.reply_text(
                "<b> ❲❓❳ - Eu esperava um texto ou documento contendo as CCs.</b>", quote=True
            )
            continue
        ...
        if not total:
            text = (
                "<b> ❲❓❳ - Não encontrei CCs na sua mensagem. Envie elas como texto ou arquivo.</b>"
            )
        else:
            text = f"✅ {success} CCs adicionadas com sucesso. Repetidas/Inválidas: {(total - success)}"
    ...
    await m.reply_text(
        "<b>❲❓❳ - Cancelado com Sucesso/b>", reply_markup=ReplyKeyboardRemove()
    )
